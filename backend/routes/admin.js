  ...
  try {
    ...
    res.json({
      ...
      reports: reports.map((r) => ({
        ...
        weatherConditions: r.weatherConditions || null, // WEATHER FEATURE
      })),
    });
  } catch (error) {
...
// DELETE /api/admin/users/:id - Permanently delete a user and all their reports (USER DELETION FEATURE)
router.delete('/users/:id', async (req, res) => {
  try {
    ...
    // Try to find in User (community) collection first
    let user = await User.findById(id);
    let userType = 'community';
    ...
    const deleteReportsResult = await Report.deleteMany({
      $or: [
        { userId: id },
        { userId: id.toString() },
        { username: username },
    ...
    // Permanently delete user from correct collection
    ...
    res.json({
      ...
      deletedUser: {
        ...
        username: username,
        userType: userType,
        reportsDeleted: deleteReportsResult.deletedCount,
        deletedBy: adminUsername,
        deletedAt: new Date(),
      },
    });
